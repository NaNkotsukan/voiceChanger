...
class GAU(Chain):
    def __init__(self, in_channels, out_channels, dilate):
        ...
        with self.init_scope():
            self.dilate = dilate
            if not dilate:
                self.convT = L.Convolution2D(in_channels, out_channels //8, ksize=(1, 2), pad=1)
                self.convS = L.Convolution2D(in_channels, out_channels //8, ksize=(1, 2), pad=1)
            else:
                self.convT = L.DilatedConvolution2D(in_channels, out_channels, ksize=(1, 3), dilate=(0, dilate), pad=(0, dilate+1))
                self.convS = L.DilatedConvolution2D(in_channels, out_channels, ksize=(1, 3), dilate=(0, dilate), pad=(0, dilate+1))
...
class ResBlock2(Chain):
    def __init__(self, in_channels, out_channels, dilate, resSize=8):
